...
|burn|-|-|-|-|-|( - ) Subtract to the Address Balance a specific amount (if owner only on available amount)|-|-|-|Read|-|-|-|-|-|-|
|burnAndReceiveNFT|-|-|-|-|-|( - ) Subtract tokens for the integer part of the amount/ratio division (if owner only on available amount)|Read|Read|Read|Read|( + ) Add amounts/ratio NFTs|Read|-|-|-|-|-|
...
We used hardhat to develop and test all smart contracts. To be able to run by yourself all test clone the repository and install all necessaries dependencies. After that, please run:
```
npx hardhat test
```
You will be able to check all tests we are developed.